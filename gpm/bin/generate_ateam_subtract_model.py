...
from mwa_pb_lookup.lookup_beam import beam_lookup_1d as gpm_beam_lookup
from gpm.bin.beam_value_at_radec import parse_metafits, beam_value
from gpm.db.check_src_fov import check_coords

# TODO: Move to a proper GPM location
...
# TODO : Move these to a more meaningful location in the GPM package
...
def check_coords_mask(w: WCS, coords: SkyCoord, border_size: int) -> bool:
    ...
    Args:
        w (WCS): Assumed GPM WCS template used for images
...
def casa_clean_script(
    ...
    Args:
        ...
        metafits (str): the metafits file used throughout processing, expected to confirm to gpm typical usage
...
def ateam_model_creation(
    ...
    Args:
        ...
        ggsm (str): Path to GPM global sky model
        ...
        check_fov (bool, optional): Consider whether a A-Team source is within the GPM image before searching for components. Defaults to False.
    ...
    for src in sources:
        response = gpm_beam_lookup(src.pos.ra.deg, src.pos.dec.deg, grid, time, freq)
        ...
        if mode == "casa":
            ...
            if apply_beam:
                ...
                comps_response = gpm_beam_lookup(
...
if __name__ == "__main__":
    ...
    try:
        GGSM = f"{os.environ['GPMBASE']}/models/GGSM.fits"
    except:
        GGSM = ""
    ...
    parser.add_argument(
        "--ggsm", default=GGSM, type=str, help="Path to the GPM Global Sky Model"
    )
    parser.add_argument(
        "--search-radius",
        default=10,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="In line with the GPM processing pipeline, if using the debug mode than the calibrated data are placed in the CORRECTED_DATA column, and these are the data that should be imaged initially. This is only relevant for the casa outlier subtraction mode. ",
    )
    parser.add_argument(
        "--source-txt",
        default=None,

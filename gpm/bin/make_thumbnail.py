...
def main(fits_image, coords, size):
    # Open the FITS file
    ...
    fig.add_subplot(111)
    plt.imshow(thumbnail, origin='lower', aspect='auto', cmap=plt.cm.viridis, extent=extent)
    plt.xlabel('RA (hours)')
    plt.ylabel('Dec (deg)')

    #plt.xlim((int(x - size/2), int(x + size/2)))
    #plt.ylim((int(y - size/2), int(y + size/2)))
    plt.tight_layout()
    plt.savefig("foo.png")
...
if __name__ == "__main__":
    ...
    # Start main
    main(args.fits_image, coords, args.size)

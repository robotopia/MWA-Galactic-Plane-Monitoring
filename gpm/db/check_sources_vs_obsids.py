...
import json
import gpm.db.mysql_db as mdb
import numpy as np
import argparse

from multiprocessing import Pool

from gpm.db.populate_sources_table import Source
from gpm.db.check_src_fov import check_coords
from gpm.bin.beam_value_at_radec import beam_value
...
def insert_app(obs_id, source, appflux, infov, cur):
    ...
    Arguments:
        ...
        cur {mysql.connector.cursor.MySQLCursor} -- Open cursor to GPM database
    """
    cur.execute("""
...
def create_peel_model(sources):
    ...
    parts of the visibility data. Assumes access to the GPM sky model.  
...
def check_obsid_peel(obsid, local_sky, check_exists=True, min_threshold=0.):
    ...
    Arguments:
        ...
        local_sky {astropy.table.Table} -- table with columns following the GPM sky model definition.  
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Calculates and stores the apparent brightness of sources (stored in the GPM database)")

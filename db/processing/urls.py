...
urlpatterns = [
    ...
    re_path(r'^set_epoch_cal/(?P<pipeline>\w+)/(?P<epoch>Epoch[0-9]+)/(?P<user>\w+)$', views.setEpochCal, name="set_epoch_cal"),
    re_path(r'^source_finder$', views.sourceFinder, name="source_finder"),
    re_path(r'^backups/(?P<epoch>Epoch[0-9]+)$', views.backupView, name="backup_view"),
